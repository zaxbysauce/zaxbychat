...
import { labelContentByAgent } from '@librechat/agents';
...
/**
 * Options for {@link createMultiAgentMapper}.
 */
export interface MultiAgentMapperOptions {
  /**
   * Phase 4 council-mode flag. When true, parallel-execution parts are
   * retained for every leg (not filtered to the primary agent). Default false
   * preserves the existing `addedConvo` behavior used by all current callers.
   */
  retainAllLegs?: boolean;
}
 ...
 *   UNLESS `options.retainAllLegs === true`, in which case all legs' parts are kept
 * - For handoffs (agentId without groupId): keeps all content from all agents
 * - For multi-agent: applies agent labels to content
 *
 * The key distinction:
 * - Parallel execution (addedConvo / council): Parts have both agentId AND groupId
 * - Handoffs: Parts only have agentId, no groupId
 */
export function createMultiAgentMapper(
  primaryAgent: Agent,
  agentConfigs?: Map<string, Agent>,
  options?: MultiAgentMapperOptions,
) {
  const retainAllLegs = options?.retainAllLegs === true;
  ...
  return (message: TMessage): TMessage => {
    ...
    try {
      ...
      for (const part of message.content) {
        ...
        const shouldInclude =
          retainAllLegs || !isParallelPart || !agentId || agentId === groupPrimary;
